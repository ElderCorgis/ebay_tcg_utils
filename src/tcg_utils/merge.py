...
The extracted header is cached in ~/.tcg_utils/header.png so it only needs
to be re-parsed when the template changes.

Usage:
    # First run — extract header from template and cache it
    tcg-merge orders.csv template.pdf -o packing_slips.pdf

    # Subsequent runs — skip extraction, use cached header.png
    tcg-merge orders.csv -o packing_slips.pdf

    # Force re-extraction (e.g. after updating your store branding)
    tcg-merge orders.csv template.pdf --refresh-header
...
def get_config_dir() -> Path:
    """Return ~/.tcg_utils, creating it if needed."""
    d = Path.home() / ".tcg_utils"
    d.mkdir(exist_ok=True)
    return d


def get_header_cache() -> Path:
    return get_config_dir() / "header.png"
...
def extract_header(template_pdf: str, header_height_pts: float) -> str:
    ...
    save it to the header cache, and return the cache path.
    """
...
_HIGHLIGHT_PLACEHOLDER = (
    "Reverse\x1fHolo"  # \x1f is not whitespace; prevents wrap splitting the phrase
)


def _draw_item_line(
    c: canvas.Canvas, x: float, y: float, text: str, size: int = 10
) -> None:
...
_SKU_MAX_WIDTH = (
    _COL_PRICE - _COL_SKU - 32
)  # leave room for price text (~20-27pt) + gap


def _wrap_to_width(text: str, font: str, size: int, max_width: float) -> list:
...
def build_pdf(
    ...
    for buyer_orders in groups.values():
        for orders in group_orders(buyer_orders):
            ...
            for order_num, rows in orders.items():
                # eBay puts an aggregate summary row first; draw individual items first, summary last
                display_rows = rows[1:] + rows[:1] if len(rows) > 1 else rows
                for row in display_rows:
                    ...
                    wrapped = wrap(
                        title.replace(_HIGHLIGHT, _HIGHLIGHT_PLACEHOLDER), 44
                    )
                    sku_lines = _wrap_to_width(sku, "Helvetica", 9, _SKU_MAX_WIDTH)
                    n_lines = max(len(wrapped), len(sku_lines))
                    for i in range(n_lines):
                        item_line = wrapped[i] if i < len(wrapped) else ""
                        sku_line = sku_lines[i] if i < len(sku_lines) else ""
                        if i == 0:
                            c.setFont("Helvetica", 9)
                            c.drawString(_COL_ORDER, y, order_num)
                            if item_line:
                                _draw_item_line(c, _COL_ITEM, y, item_line)
                            c.setFont(
                                "Helvetica", 9
                            )  # _draw_item_line leaves canvas at 10pt
                            c.drawString(_COL_QTY, y, qty)
                            c.drawString(_COL_SKU, y, sku_line)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Required on first run or with --refresh-header; otherwise the cached header is reused."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--refresh-header",
        action="store_true",
        help="Re-extract the header from template_pdf even if a cached header already exists",
    )
    parser.add_argument(
        "--header-height",
        type=float,
    ...
    needs_extract = args.refresh_header or not header_cache.exists()

    if needs_extract:
        if not args.template_pdf:
            parser.error(
                "template_pdf is required when no cached header exists or --refresh-header is set"
            )
    ...
    print(
        ...
        + (
            f" ({split_buyers} buyer(s) split due to >${PRICE_LIMIT:.0f} limit)"
            if split_buyers
            else ""
        )
    )
